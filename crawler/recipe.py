...
# 레시피 ID 추출 (cat4 기준)
def get_recipe_ids_by_cat4(cat4, max_count=5):
    ids = set()
    page = 1
...
for category_name, cat4 in categories.items():
    ...
    recipe_ids = get_recipe_ids_by_cat4(cat4, max_count=5)
...
# HTML 파일 저장 디렉토리 설정
html_base_dir = os.path.join(base_dir, "html_files")
os.makedirs(html_base_dir, exist_ok=True)

# HTML 파일을 저장하고, URL을 저장
for recipe in all_recipes:
    recipe_id = recipe["id"]
    html_path = os.path.join(html_base_dir, f"{recipe_id}.html")
    with open(html_path, "w", encoding="utf-8") as f:
        f.write(requests.get(f"{BASE_URL}/recipe/{recipe_id}").text)
